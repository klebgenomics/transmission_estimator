...
plot_snp_vs_temporal_dist <- function(distance_data, snp_column = 'dist', temporal_dist_column = 'days', 
                                      x_label = "Pairwise SNP distances", 
                                      y_label = "Pairwise temporal distances (days)",
                                      plot_title = "Distribution of pairwise distances"){
    ...
    # plot
    p <- ggpubr::ggscatter(distance_data, x = snp_column, y = temporal_dist_column, 
                      cor.coef = T, cor.method = 'pearson', color = "#808080",
                      xlab = x_label, ylab = y_label) +
        ggplot2::geom_smooth(method = "lm", formula = y ~ x, color = "#2a77be")
    snp_vs_temporal_dist_plot <- ggExtra::ggMarginal(p, type = "histogram", fill = "#2a77be")
