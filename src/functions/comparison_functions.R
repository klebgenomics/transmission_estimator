...
set_comparison_column <- function(
        combined_datasets_df, user_comparison_var, dataset_id_col="Study"){
    # Vector of groups to iterate
    ...
    d <- furrr::future_map_dfr(unique_datasets, ~{
        ...
        # Stratify by user_comparison_var if exists (not NAs)
        if (all(is.na(dataset_df[[user_comparison_var]]))){
            comp_var <- dataset_id_col
            msg <- glue::glue('Selected stratification variable ({user_comparison_var}) 
        ...
        } else {comp_var <- user_comparison_var}
